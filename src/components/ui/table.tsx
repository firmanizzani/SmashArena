...
>(({ className, ...props }, ref) => (
  <thead ref={ref} className={cn("bg-black/35 [&_tr]:border-b", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn("border-t border-border bg-black/35 font-medium [&>tr]:last:border-b-0", className)}
    {...props}
  />
));
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "border-b border-border/70 transition-colors hover:bg-white/[0.03] data-[state=selected]:bg-accent",
        className,
      )}
      {...props}
    />
